...
PRICE_CACHE = {}
waiting_for = {}
last_message_id = {}      # stores the last bot message ID per user
cooldowns = {}
...
def delete_previous(chat_id):
    """Delete the last bot message for this user, if any."""
    prev = last_message_id.get(chat_id)
    if prev:
        try:
...
def send_new(chat_id, text, reply_markup=None):
    """Delete previous message, then send a new one and store its ID."""
    delete_previous(chat_id)
...
def get_price(symbol):
    ...
    data = r.json()
    if "price" not in data: return None,None
...
def on_ws_error(ws, err): print(f"WS error: {err}")
def on_ws_close(ws, *args): print("WS closed")
def on_ws_open(ws): print("WS connected")
...
def start(msg):
    send_new(msg.chat.id, "🤖 *Persona* — your crypto assistant\n\nChoose an option:", reply_markup=main_menu())
...
def callback(call):
    ...
    try:
        if data == "back_main":
            ...
            send_new(cid, "🤖 *Persona* — your crypto assistant\n\nChoose an option:", reply_markup=main_menu())
            bot.answer_callback_query(call.id)

        elif data == "menu_price":
            send_new(cid, "💰 *Select a coin or search:*", reply_markup=price_menu())
            bot.answer_callback_query(call.id)

        elif data == "search_coin":
            waiting_for[cid] = "price"
            send_new(cid, "🔍 *Type any coin symbol:*\nExample: `PEPE`, `WIF`", reply_markup=back_button())
            bot.answer_callback_query(call.id)

        elif data == "menu_alerts":
            send_new(cid, "🔔 *Quick Alerts* — tap to set or create custom:", reply_markup=alerts_menu())
            bot.answer_callback_query(call.id)

        elif data == "custom_alert":
            waiting_for[cid] = "alert"
            send_new(cid, "✏️ *Format:* `COIN > price` or `COIN < price`\nExample: `BTC > 95000`", reply_markup=back_button())
            bot.answer_callback_query(call.id)
            ...
            kb.row(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            send_new(cid, text, reply_markup=kb)
            bot.answer_callback_query(call.id)
            ...
            active = [a for a in alerts if a["chat_id"]==cid and a["active"]]
            if not active:
                send_new(cid, "📋 No more active alerts.", reply_markup=back_button())
            else:
                text = "📋 *Active Alerts:*\n\n"
                kb = InlineKeyboardMarkup()
                for a in active:
            ...
            p,ch = get_price(sym)
            if p is None:
                send_new(cid, f"❌ Could not fetch *{sym}*.", reply_markup=price_menu())
            else:
                arrow = "🟢 ▲" if ch>=0 else "🔴 ▼"
            ...
            send_new(cid, "🔎 *Coin Info — Select or search:*", reply_markup=info_coins_menu())
            bot.answer_callback_query(call.id)
            ...
            if not info:
                send_new(cid, f"❌ Couldn't fetch info for *{sym}*.", reply_markup=info_coins_menu())
            else:
            ...
            send_new(cid, "💱 *Multi-Currency Price — Select or search:*", reply_markup=multi_coins_menu())
            bot.answer_callback_query(call.id)
            ...
            if not prices:
                send_new(cid, f"❌ Couldn't fetch *{sym}*.", reply_markup=multi_coins_menu())
            else:
                flags = {"usd":"🇺🇸 $","eur":"🇪🇺 €","gbp":"🇬🇧 £","jpy":"🇯🇵 ¥","cny":"🇨🇳 ¥","aed":"🇦🇪 د.إ","try":"🇹🇷 ₺"}
                text = f"💱 *{sym} Price*\n\n"
                for cur,flag in flags.items():
                    p = prices.get(cur)
                    if p: text += f"{flag} {p:,.4f}\n"
                send_new(cid, text, reply_markup=multi_coins_menu())

        elif data == "menu_scan":
            waiting_for[cid] = "scan"
            send_new(cid, "🛡 *CA Scanner*\n\nPaste contract address (ETH, BSC, Solana):", reply_markup=back_button())
            bot.answer_callback_query(call.id)
...
def text_input(msg):
    global alert_id_counter, ws_restart_required
    cid = msg.chat.id
    if cid not in waiting_for: return
    ...
    try:
        if mode == "price":
            ...
            if p is None:
                send_new(cid, f"❌ Could not fetch *{text.upper()}*.", reply_markup=price_menu())
            else:
                arrow = "🟢 ▲" if ch>=0 else "🔴 ▼"
                send_new(cid, f"*{text.upper()}*\n💵 ${p:,.4f}\n{arrow} {abs(ch):.2f}% (24h)", reply_markup=price_menu())

        elif mode == "info":
            info = get_coin_info(text.upper())
            if not info:
                send_new(cid, f"❌ *{text.upper()}* not found.", reply_markup=info_coins_menu())
            else:
                supply = f"{info['supply']:,.0f}" if info['supply'] else "N/A"
                max_s = f"{info['max_supply']:,.0f}" if info['max_supply'] else "∞"
                send_new(cid,
            ...
            if not prices:
                send_new(cid, f"❌ *{text.upper()}* not found.", reply_markup=multi_coins_menu())
            else:
                flags = {"usd":"🇺🇸 $","eur":"🇪🇺 €","gbp":"🇬🇧 £","jpy":"🇯🇵 ¥","cny":"🇨🇳 ¥","aed":"🇦🇪 د.إ","try":"🇹🇷 ₺"}
                out = f"💱 *{text.upper()} Price*\n\n"
                for cur,flag in flags.items():
                    p = prices.get(cur)
                    if p: out += f"{flag} {p:,.4f}\n"
                send_new(cid, out, reply_markup=multi_coins_menu())

        elif mode == "scan":
            res = scan_ca(text)
            if not res:
                send_new(cid, "❌ Contract not found or unsupported chain.", reply_markup=back_button())
            else:
                def f(v): return "⚠️ Yes" if v=="1" else "✅ No" if v=="0" else "❓ Unknown"
                send_new(cid,
                         f"🛡 *CA Scan: {res.get('token_name','Unknown')} ({res.get('token_symbol','?')})*\n\n"
                         f"🍯 Honeypot: {f(res.get('is_honeypot','?'))}\n"
                         f"🖨 Mintable: {f(res.get('is_mintable','?'))}\n"
                         f"🔁 Proxy: {f(res.get('is_proxy','?'))}\n"
                         f"📂 Open Source: {f(res.get('is_open_source','?'))}\n"
            ...
            if len(parts)!=3 or parts[1] not in ['>','<']:
                send_new(cid, "❌ Wrong format. Use: `BTC > 70000`", reply_markup=alerts_menu())
                return
            sym = parts[0].upper()
            dir = parts[1]
...
# ================= START BOT =================
print("🚀 Persona — One message at a time, previous message auto‑deleted")
while True:
    try:
